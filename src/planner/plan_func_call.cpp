...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if ((func_name == "lower" || func_name == "upper") && args.size() == 1) {
    StringExpressionType type = (func_name == "lower" ? StringExpressionType::Lower : StringExpressionType::Upper);
    return std::make_shared<StringExpression>(std::move(args.at(0)), type);
  }
